...
for idx in casing_idx:
    ...
    r["city"] = (
        r["city"].upper()
        if variant == "upper"
        else (r["city"].lower() if variant == "lower" else " " + r["city"] + " ")
    )
    ...
    r["category"] = (
        r["category"].upper()
        if variant2 == "upper"
        else (r["category"].lower() if variant2 == "lower" else " " + r["category"] + " ")
    )
...
dup_pool = [
    i for i in range(len(raw_rows))
    if i not in casing_idx and i not in null_idx and i not in outlier_idx
]
